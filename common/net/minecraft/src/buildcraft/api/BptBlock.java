 ...
 * blueprints:
 * 
 * - what items needs to be used to create that block
 * - how the block has to be built on the world
 * - how to rotate the block
 * - what extra data to store / load in the blueprint
 * 
 ...
 * 
 * Blueprints perform "id translation" in case the block ids between a 
 * blueprint and the world installation are different. In order to translate
 * block ids, blocks needs to be uniquely identified. By default, this 
 * identification is done by:
 * 
 * - the block simple class name
 * - the tile simple class name (if any)
 * - the block name
 * 
 ...
 * perform a search through the block list, and upon matching signature, it
 * will translate all blocks ids of the blueprint to the installation ones. If
 * no such block id is found, BuildCraft will assume that the block is not
...
public class BptBlock {
	 ...
	 * requirements are met, they will be removed all at once from the 
	 * builder, before calling buildBlock.
	 */
	...
	/**
	 * This is called each time an item matches a reqquirement, that is:
	 *    (req id == stack id) for damageable items
	 *    (req id == stack id && req dmg == stack dmg) for other items
	 * by default, it will increase damage of damageable items by the amount
	 * of damage of the requirement, and remove the intended amount of non
	 * damageable item.
	 * 
	 * Client may override this behavior for default items. Note that this 
	 * subprogram may be called twice with the same parameters, once with a
	 * copy of requirements and stack to check if the entire requirements can
	 * be fullfilled, and once with the real inventory. Implementer is 
	 * responsible for updating req (with the remaining requirements if any)
	 * and stack (after usage)
	 */
	...
	/**
	 * Return true if the block on the world correspond to the block stored
	 * in the blueprint at the location given by the slot. By default, this
	...
	public void buildBlock(BptSlotInfo slot, IBptContext context) {
		...
		context.world().setBlockMetadataWithNotify(slot.x, slot.y,
				slot.z, slot.meta);
	...
	/**
	 * Initializes a slot from the blueprint according to an objet placed on 
	 * {x, y, z} on the world. This typically means adding entries in slot.cpt.
	 * Note that "id" and "meta" will be set automatically, corresponding to
	 * the block id and meta.
	 * 
	 * By default, if the block is a BlockContainer, tile information will be
	 * to save / load the block.
	 */
	public void initializeFromWorld(BptSlotInfo slot, IBptContext context, int x, int y, int z) {
		...
		if (Block.blocksList[slot.blockId] != null) {
			ArrayList<ItemStack> req = Block.blocksList[slot.blockId]
					.getBlockDropped(context.world(), x, y, z,
	...
	/**
	 * By default, block class name, block tile name and block name are used
	 * to define block signature. Overriding this subprogram may allow to
	 * replace some of these with stars, specify the mod that this block kind
	 * is coming from or add custom data to the signature.
	 */
	 ...
	 * installation block - except for the default blocks which don't check 
	 * for classes.
	 * For any value, * means match with anything.
	 * For compatibilty and evolution reasons, mods may want to write a 
	 * different policy, allowing to migrate one format to the
	 * other.
	 */
