...
//                    if (type == Type.TYPE && target.getHasSubtypes())
        ...
        public static Line fromNBT(CompoundNBT data) {
            Line line = new Line();

            if (data != null && data.contains("st")) {
                ListNBT l = data.getList("st", 10);
...
//                    i.getSubItems(ItemGroup.TAB_SEARCH, examples);
                    i.fillItemCategory(ItemGroup.TAB_SEARCH, examples);
    ...
    public static Line[] getLines(@Nonnull ItemStack item) {
        CompoundNBT data = NBTUtilBC.getItemData(item);
        if (data.contains("written") && data.contains("lines")) {
            ListNBT list = data.getList("lines", 10);
    ...
    public static void saveLines(@Nonnull ItemStack stackList, Line[] lines) {
        ...
        if (hasLine) {
            CompoundNBT data = NBTUtilBC.getItemData(stackList);
            data.putBoolean("written", true);
            ListNBT lineList = new ListNBT();
