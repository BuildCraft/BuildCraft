...
public enum IntegrationRecipeRegistry implements IIntegrationRecipeRegistry {
    ...
    @Override
    public IntegrationRecipe getRecipeFor(@Nonnull ItemStack target, @Nonnull List<ItemStack> toIntegrate) {
    ...
    public static boolean matches(IntegrationRecipe recipe, @Nonnull ItemStack target, List<ItemStack> toIntegrate) {
        if (!StackUtil.contains(recipe.target, target)) {
            return false;
        }
        List<ItemStack> toIntegrateCopy = toIntegrate.stream().filter(Objects::nonNull).collect(Collectors.toList());
