...
//        return new DistillationRecipe(powerRequired, in, outGas, outLiquid);
    ...
    private static class SingleRegistry<R extends IRefineryRecipe> implements IRefineryRegistry<R> {
        ...
        private SingleRegistry(IRecipeType<R> type) {
            this.type = type;
        }

        @Override
//        public Stream<R> getRecipes(Predicate<R> filter)
        public Stream<R> getRecipes(World world, Predicate<R> filter) {
...
//            Collection<? extends IRecipe> recipes = world.getRecipeManager().byType(type).values();
...
//        public Collection<R> getAllRecipes()
        public Collection<R> getAllRecipes(World world) {
//            return allRecipes;
            Collection<R> ret = Lists.newArrayList();
            ret.addAll(unregisteredRecipes);
//            ret.addAll(world.getRecipeManager().byType(type).values().stream().map(r -> (R) r).toList());
...
//        public R getRecipeForInput(@Nullable FluidStack fluid)
        public R getRecipeForInput(World world, @Nullable FluidStack fluid) {
            if (fluid == null) {
                return null;
            }
...
//            super(in);
        ...
        @Override
        public IRecipeSerializer<IDistillationRecipe> getSerializer() {
...
//            super(in, out, heatFrom, heatTo);
        ...
        @Override
        public IRecipeSerializer<IHeatExchangerRecipe> getSerializer() {
