...
public abstract class GuidePart {
    ...
    protected void renderTextLine(MatrixStack poseStack, String text, int x, int y, int colour) {
...
//        GlStateManager.color(1, 1, 1);
    ...
    public abstract PagePosition renderIntoArea(MatrixStack poseStack, int x, int y, int width, int height, PagePosition current, int index);

    /** Like {@link #renderIntoArea(MatrixStack, int, int, int, int, PagePosition, int)} but for a mouse click. */
    public abstract PagePosition handleMouseClick(MatrixStack poseStack, int x, int y, int width, int height, PagePosition current, int index,
    ...
    protected PagePosition renderLine(MatrixStack poseStack, PagePosition current, PageLine line, int x, int y, int width, int height,
                                      int pageRenderIndex) {
        wasHovered = false;
        ...
        ITextComponent toRender = line.text;
        ...
        while (next != null) {
            ...
            if (render) {
                ...
                if (wasHovered) {
                    if (line.link) {
                        AbstractGui.fill(poseStack, _x - 2, _y - 2, _x + _w + 2, _y + 1 + neededSpace, 0xFFD3AD6C);
                    }
                    renderTooltip();
                }
    ...
    protected PagePosition renderLines(MatrixStack poseStack, Iterable<PageLine> lines, PagePosition part, int x, int y, int width, int height,
                                       int index) {
    ...
    protected PagePosition renderLines(MatrixStack poseStack, Iterable<PageLine> lines, int x, int y, int width, int height, int index) {
