...
public class SpriteNineSliced {
    ...
    public void draw(IGuiArea element, MatrixStack poseStack) {
        draw(poseStack, element.getX(), element.getY(), element.getWidth(), element.getHeight());
    }

    public void draw(MatrixStack poseStack, double x, double y, double width, double height) {
        sprite.bindTexture();
...
//        vb.setTranslation(0, 0, 0);
    ...
    private void quad(BufferBuilder vb, MatrixStack poseStack, double[] x, double[] y, double[] u, double[] v, int xIndex, int yIndex) {
        int xis = xIndex;
        int xIB = xIndex + 1;
    ...
    private void vertex(BufferBuilder vb, Entry pose, double x, double y, double texU, double texV) {
//        vb.pos(x, y, 0);
