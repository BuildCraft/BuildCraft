...
public enum OilGenerator implements IWorldGenerator {
    ...
    public static List<OilGenStructure> getStructures(World world, int cx, int cz) {
        ...
        if (OilPopulate.INSTANCE.excludedBiomes.contains(biome.getBiomeName())) {
            return ImmutableList.of();
        }

        boolean oilBiome = OilPopulate.INSTANCE.surfaceDepositBiomes.contains(biome.getBiomeName());

        double bonus = oilBiome ? 3.0 : 1.0;
        if (OilPopulate.INSTANCE.excessiveBiomes.contains(biome.getBiomeName())) {
            bonus *= 30.0;
        }
        final GenType type;
