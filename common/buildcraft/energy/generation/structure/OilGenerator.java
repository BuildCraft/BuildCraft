...
public class OilGenerator extends StructureStart<OilFeatureConfiguration> {
    ...
    public OilGenerator(Structure<OilFeatureConfiguration> feature, int chunkX, int chunkZ, MutableBoundingBox box, int references, long seed) {
        super(feature, chunkX, chunkZ, new MutableBoundingBox(box), references, seed);
    }
...
//        int maxHeight = Math.max(super.boundingBox.y0, super.boundingBox.y1);
        int maxHeight = 384;
...
//        GenType type = getPieceTypeByRand(rand, biome, cx, cz, xForGen, zForGen, true);
        OilFeatureConfiguration.Info info = config.get(chunkPos);
        if (info == null) {
        ...
        SharedSeedRandom rand = info.oilRand;
        ...
        OilStructure structure = createStructureByType(type, rand, xForGen, zForGen, minHeight, box);
        // type == NONE -> null
        if (structure != null) {
            super.pieces.add(structure);
    ...
    public static GenType getPieceTypeByRand(Random rand, Biome biome, int cx, int cz, int x, int z, boolean log) {
        ...
        // 1.16.5: no biome tags and cannot get dimension
        if (biome.getBiomeCategory() == Biome.Category.THEEND && (Math.abs(x) < 1200 || Math.abs(z) < 1200)) {
            if (DEBUG_OILGEN_BASIC & log) {
                BCLog.logger.info(
                        "[energy.oilgen] Not generating oil in chunk " + cx + ", " + cz
    ...
    public static OilStructure createStructureByType(final GenType type, Random rand, int x, int z, int worldBottomHeight, Box box) {
