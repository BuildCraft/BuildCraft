...
public class GuideManager {
    ...
    public void registerAllBlocks() {
        ...
        for (ResourceLocation location : Block.blockRegistry.getKeys()) {
            ...
            if (domain.equalsIgnoreCase(prefix)) {
                registerBlock(Block.blockRegistry.getObject(location));
            }
        }
    }
    ...
    public void registerAllItems(boolean andItemBlocks) {
        ...
        String prefix = container.getModId();
        for (ResourceLocation location : Item.itemRegistry.getKeys()) {
            if (Block.getBlockFromItem(Item.itemRegistry.getObject(location)) != null && !andItemBlocks) {
                continue;
            }
            String domain = location.getResourceDomain();
            if (domain.equalsIgnoreCase(prefix)) {
                registerItem(Item.itemRegistry.getObject(location));
            }
        }
    }
