...
//                            .getInterpolatedV(8 + c * (x[(i + 1) % 4] * 2 - 1) + s * (z[(i + 1) % 4] * 2 - 1)));
                    ...
                    putVertex(builder2, side, x[a], y[a], z[a], topSprite.getInterpolatedU(8 + c * (x[a] * 2 - 1) + s * (z[a] * 2 - 1)), topSprite
                            .getInterpolatedV(8 + c * (x[(a + 1) % 4] * 2 - 1) + s * (z[(a + 1) % 4] * 2 - 1)));
                }
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        for (int j = 0; j < 4; j++) {
                            ...
                            if (gas && z[l] == 0) yl = 1;
                            putVertex(builder, side, x[(i + x[l]) % 4], yl, z[(i + x[l]) % 4], flowing.getInterpolatedU(x[l] * 8), flowing
                                    .getInterpolatedV((gas ? yl : 1 - yl) * 8));
                        }
                        q[k] = builder.build();
                    }
