		...
		if (station == robot.getLinkedStation() && station == robot.getDockingStation()) {
			...
			if (station.takeAsMain(robot)) {
				startDelegateAI(new AIRobotGotoBlock(robot,
						station.x() + station.side.getFrontOffsetX() * 2,
						station.y() + station.side.getFrontOffsetY() * 2,
						station.z() + station.side.getFrontOffsetZ() * 2));
			} else {
				terminate();
			}
		}
	...
	public void delegateAIEnded(AIRobot ai) {
		if (ai instanceof AIRobotGotoBlock) {
			startDelegateAI(new AIRobotStraightMoveTo(robot,
					station.x() + 0.5F + station.side.getFrontOffsetX() * 0.5F,
					station.y() + 0.5F + station.side.getFrontOffsetY() * 0.5F,
					station.z() + 0.5F + station.side.getFrontOffsetZ() * 0.5F));
		} else {
			robot.dock(station);
			station = null;
			terminate();
