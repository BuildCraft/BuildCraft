...
public class PipeTransportItems extends PipeTransport {
	...
	public void groupEntities() {
		...
		if (entities.size() < 2) return; // nothing groupable
		...
		Collections.sort(entities, new Comparator<EntityData>() {
			...
			public int compare(EntityData a, EntityData b) {
				...
				ItemStack stackA = a.item.getItemStack();
				ItemStack stackB = b.item.getItemStack();
				int itemA = (stackA.itemID << 16) | stackA.getItemDamage();
				int itemB = (stackB.itemID << 16) | stackB.getItemDamage();
		...
		// group the entities
		ItemStack firstItemStack = entities.get(0).item.getItemStack();
		int matchStart = 0;
		int lastId = (firstItemStack.itemID << 16) | firstItemStack.getItemDamage();

		for (int i = 1; i < entities.size(); i++) {
			ItemStack itemStack = entities.get(i).item.getItemStack();
			int id = (itemStack.itemID << 16) | itemStack.getItemDamage();
