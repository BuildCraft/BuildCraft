        ...
        while (!queue.isEmpty()) {
            ...
            if (!walked.contains(element)) {
                if (!holdersCache.containsKey(element.blockPos)) {
                    TileEntity tile = wireSystems.world.getBlockEntity(element.blockPos);
...
//                            players -> !players.isEmpty()
                            players -> players.anyMatch(p -> true)
                    );
        }
        return false;
    }
    ...
    public CompoundNBT writeToNBT() {
        ...
        ListNBT elementsList = new ListNBT();
    ...
    public WireSystem(CompoundNBT nbt) {
        ListNBT elementsList = nbt.getList("elements", Constants.NBT.TAG_COMPOUND);
    ...
    public static class WireElement {
        ...
        public WireElement(CompoundNBT nbt) {
