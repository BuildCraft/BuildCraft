...
public enum PipeExtensionManager implements IPipeExtensionManager {
    ...
    public boolean requestPipeExtension(World world, BlockPos pos, EnumFacing dir, IStripesActivator stripes, ItemStack stack) {
        if (world.isRemote || BCStackHelper.isEmpty(stack) || !(stack.getItem() instanceof IItemPipe)) {
            return false;
        }
    ...
    private void retract(World w, PipeExtensionRequest r) {
        ...
        List<ItemStack> stacksToSendBack = Lists.newLinkedList();
        ...
        if (!canceled) {
            ...
            if (canceled = placeEvent.isCanceled()) {
                ...
                canceled = !BlockUtil.breakBlock((WorldServer) w, r.pos, Lists.newLinkedList(), r.pos, owner);
                if (canceled) {
    ...
    private void extend(World w, PipeExtensionRequest r) {
        ...
        stripesTileOld.writeToNBT(stripesNBTOld);
        BlockSnapshot blockSnapshot1 = BlockSnapshot.getBlockSnapshot(w, r.pos);
        boolean canceled = !BlockUtil.breakBlock((WorldServer) w, r.pos, Lists.newLinkedList(), r.pos, owner);
        if (canceled) {
            stacksToSendBack.add(r.stack);
    ...
    private void cleanup(World w, PipeExtensionRequest r, BlockPos p, List<ItemStack> stacksToSendBack, boolean canceled, NBTTagCompound stripesNBTOld) {
