...
public class MapWorld {
    ...
    private final HashMap<IChunk, Integer> timeToUpdate = new HashMap<IChunk, Integer>();
        ...
        if (region == null) {
            ...
            if (target.exists()) {
                try {
                    ...
                    CompoundNBT nbt = CompressedStreamTools.read(target);
                    if (nbt != null) {
    ...
    public void tick() {
        if (timeToUpdate.size() > 0) {
            synchronized (timeToUpdate) {
                ...
                Set<IChunk> chunks = new HashSet<IChunk>();
    ...
    public void updateChunkDelayed(IChunk chunk, byte time) {
        synchronized (timeToUpdate) {
