...
//        skullOverlayBox.setRotationPoint(0.0F, 0.0F, 0.0F);
    ...
    public void render(EntityRobot robot, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        if (glasses) {
            ...
            if (!robot.itemInUse.isEmpty()) {
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(robot, itemstack1, ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, false, poseStack, bufferSource, robot.level, packedLight, OverlayTexture.NO_OVERLAY, robot.getId());
                // }
    ...
    private void doRenderItem(ItemStack stack, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, EntityRobot robot) {
        ...
        Minecraft.getInstance().getItemRenderer().renderStatic(robot, stack, ItemTransforms.TransformType.GROUND, false, poseStack, bufferSource, robot.level, packedLight, OverlayTexture.NO_OVERLAY, robot.getId());
