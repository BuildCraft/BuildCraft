...
public class GuiZonePlanner extends GuiBC8<ContainerZonePlanner> {
    ...
    static {
        RenderType.State rendertype$state = RenderType.State.builder()
                ...
                .setLineState(new RenderState.LineState(OptionalDouble.of(2.0f)))
                .createCompositeState(false);
        CUBOID_LINES_RENDER_TYPE = RenderType.create("buildcraft_zone_planner_gui_lines", DefaultVertexFormats.POSITION_COLOR, GL11.GL_LINE_STRIP, 256, false, true, rendertype$state);
    }
...
//        ICON_GUI.drawAt(mainGui.rootElement);
    ...
    private BlockPos rayTrace(double screenX, double screenY, MatrixStack modelViewStack, Matrix4f projectionMatrix) {
        BlockPos found = null;
...
//        GlStateManager.glGetInteger(GL11.GL_VIEWPORT, viewportBuffer);
        int[] viewport = { GlStateManager.Viewport.INSTANCE.x, GlStateManager.Viewport.INSTANCE.y, GlStateManager.Viewport.INSTANCE.width, GlStateManager.Viewport.INSTANCE.height };
...
//        GLU.gluUnProject(screenX, screenY, 1f, modelViewBuffer, projectionBuffer, viewportBuffer, positionFarBuffer);
        Matrix4f mul = new Matrix4f(projectionMatrix);
        mul.multiply(modelViewStack.last().pose());
...
//            fontRenderer.drawString(text, x + 130, y + 130, 0x404040);
            font.drawShadow(poseStack, text, x + 130, y + 130, 0x404040, false);
        }
        int offsetX = 8;
        int offsetY = 9;
        int sizeX = 213;
...
//        GLU.gluPerspective(70.0F, (float) sizeX / sizeY, 1F, 10000.0F);
        ...
        GlStateManager._multMatrix(projectionMatrix);
...
//        GlStateManager.rotate(90, 1, 0, 0); // look down
        modelViewStack.mulPose(Vector3f.YP.rotationDegrees(90));
//        GlStateManager.pushMatrix();
...
//                        .ifPresent(GlStateManager::callList);
                        .ifPresent(vertexBuffer -> this.renderVertexBuffer(vertexBuffer, modelViewStack));
            }
        }
...
//            builder.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
            IVertexBuilder builder = Minecraft.getInstance().renderBuffers().bufferSource().getBuffer(CUBOID_LINES_RENDER_TYPE);
...
//                Tessellator.getInstance().draw();
                this.uploadBufferBuilderAndRender(Tessellator.getInstance().getBuilder(), modelViewStack);
            }
        }
//        GlStateManager.disableBlend();
        RenderUtil.disableBlend();
...
//        GlStateManager.disableBlend();
    ...
    private void renderVertexBuffer(VertexBuffer vertexBuffer, MatrixStack modelViewStack) {
        GlStateManager._color4f(1, 1, 1, 1);
        DefaultVertexFormats.POSITION_COLOR.setupBufferState(0L);
        vertexBuffer.draw(modelViewStack.last().pose(), GL11.GL_QUADS);
        VertexBuffer.unbind();
        DefaultVertexFormats.POSITION_COLOR.clearBufferState();
    }

    private void uploadBufferBuilderAndRender(BufferBuilder builder, MatrixStack modelViewStack) {
        builder.end();
        VertexBuffer vertexBuffer = new VertexBuffer(DefaultVertexFormats.POSITION_COLOR);
    ...
    private static void unproject(Matrix4f mul, float winX, float winY, float winZ, int[] viewport, Vector3f dest) {
        ...
        float invW = 1.0f / (floatBuffer.get(0 * 4 + 3) * ndcX + floatBuffer.get(1 * 4 + 3) * ndcY + floatBuffer.get(2 * 4 + 3) * ndcZ + floatBuffer.get(3 * 4 + 3));
        dest.set((floatBuffer.get(0 * 4 + 0) * ndcX + floatBuffer.get(1 * 4 + 0) * ndcY + floatBuffer.get(2 * 4 + 0) * ndcZ + floatBuffer.get(3 * 4 + 0)) * invW,
                (floatBuffer.get(0 * 4 + 1) * ndcX + floatBuffer.get(1 * 4 + 1) * ndcY + floatBuffer.get(2 * 4 + 1) * ndcZ + floatBuffer.get(3 * 4 + 1)) * invW,
                (floatBuffer.get(0 * 4 + 2) * ndcX + floatBuffer.get(1 * 4 + 2) * ndcY + floatBuffer.get(1 * 4 + 2) * ndcZ + floatBuffer.get(3 * 4 + 2)) * invW);
    }
}
