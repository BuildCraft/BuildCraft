...
//    }
    ...
    public GuiRequester(ContainerRequester container, PlayerInventory inventory, ITextComponent component) {
        ...
        if (mouseButton == 0) {
            for (int i = 0; i < container.tile.requests.size(); i++) {
                if (getArea(i).contains(mouseX, mouseY)) {
                    this.container.tile.setRequest(i, minecraft.player.inventory.getCarried());
