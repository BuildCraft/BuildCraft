...
//    public void addInformation(ItemStack stack, Player player, List list, boolean advanced)
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> list, ITooltipFlag flag) {
        ...
        if (boardNBT != RedstoneBoardRegistry.instance.getEmptyRobotBoard()) {
            ...
            enInfo = (pct >= 80 ? TextFormatting.GREEN : (pct >= 50 ? TextFormatting.YELLOW : (pct >= 30 ? TextFormatting.GOLD
                    : (pct >= 20 ? TextFormatting.RED : TextFormatting.DARK_RED)))) + enInfo;
            list.add(new StringTextComponent(enInfo));
        }
    }
    ...
    protected void addSubItems(ItemGroup tab, NonNullList<ItemStack> itemList) {
...
//        }
    ...
    public long receivePower(ItemStack container, long maxReceive, boolean simulate) {
        CompoundNBT cpt = getNBT(container);
    ...
    private static CompoundNBT getNBT(ItemStack stack) {
...
//    }
    ...
    private static long getEnergy(CompoundNBT cpt) {
        return cpt.getLong(MjBattery.NBT_STORED);
    }

    private static void setEnergy(CompoundNBT cpt, long energy) {
