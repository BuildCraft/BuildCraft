...
public class PluggableRobotStation extends PipePluggable implements IMjReceiver, IMjReadable, IDebuggable, IDockingStationProvider {
    ...
    public enum EnumRobotStationState implements IStringSerializable {
        None,
        Available,
        Reserved,
        Linked;
    ...
    public PluggableRobotStation(PluggableDefinition definition, IPipeHolder holder, Direction side, PacketBuffer buffer) {
        super(definition, holder, side);
        readData(buffer);
    }

    public PluggableRobotStation(PluggableDefinition definition, IPipeHolder holder, Direction side, CompoundNBT nbt) {
        super(definition, holder, side);
    }
...
//    }
    ...
    public void validate() {
        ...
        if (!isValid && !pipe.getPipeWorld().isClientSide) {
            station = (DockingStationPipe) RobotManager.registryProvider.getRegistry(pipe.getPipeWorld()).getStation(((TileEntity) pipe).getBlockPos(), direction);
...
//    }
    ...
    public void getDebugInfo(List<ITextComponent> left, List<ITextComponent> right, Direction side) {
        if (station == null) {
            left.add(new StringTextComponent("PluggableRobotStation: No station found!"));
        } else {
            refreshRenderState();
            left.add(new StringTextComponent("Docking Station (side " + side.name() + ", " + renderState.name() + ")"));
