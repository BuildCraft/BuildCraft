...
public class DockingStationPipe extends DockingStation implements IRequestProvider {
    ...
    public DockingStationPipe(IPipeHolder iPipe, Direction side) {
        super(((TileEntity) iPipe).getBlockPos(), side);
        pipe = iPipe;
    ...
    public IPipeHolder getPipe() {
        if (pipe == null) {
            TileEntity tile = world.getBlockEntity(getPos());
            if (tile instanceof IPipeHolder) {
                pipe = (IPipeHolder) tile;
            }
        }

        if (pipe == null || ((TileEntity) pipe).isRemoved()) {
    ...
    public IInventory getItemInput() {
        ...
        TileEntity connectedTile = getPipe().getPipeWorld().getBlockEntity(getPos().relative(dir));
    ...
    public IRequestProvider getRequestProvider() {
        for (Direction dir : Direction.values()) {
            ...
            TileEntity nearbyTile = getPipe().getPipeWorld().getBlockEntity(getPos().relative(dir));
