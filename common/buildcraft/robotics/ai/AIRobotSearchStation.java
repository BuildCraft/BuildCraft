...
public class AIRobotSearchStation extends AIRobot {
    ...
    public void start() {
        ...
        for (DockingStation station : robot.getRegistry().getStations()) {
            ...
            if (zone != null && !zone.contains(VecUtil.convert(station.getPos()))) {
                continue;
            }
            ...
            if (filter.matches(station)) {
                ...
                // double distance = VecUtil.getPos(robot).distSqr(station.getPos());
                double distance = VecUtil.distanceSq(VecUtil.getPos(robot), station.getPos());
