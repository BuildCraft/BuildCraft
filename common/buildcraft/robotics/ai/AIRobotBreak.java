...
//            speed = getBreakSpeed(robot, robot.getHeldItem(), state, blockToBreak);
        ...
        if (blockDamage > 1.0F) {
            ...
            if (!robot.getMainHandItem().isEmpty()) {
                FakePlayer fakePlayer = FakePlayerProvider.INSTANCE.getFakePlayer((ServerWorld) robot.level, FakePlayerProvider.NULL_PROFILE);
                if (robot.getMainHandItem().getItem().onBlockStartBreak(robot.getMainHandItem(), blockToBreak, fakePlayer)) {
                    continueBreaking = false;
                }
            }

            if (continueBreaking && BlockUtil.harvestBlock((ServerWorld) robot.level, blockToBreak, robot.getMainHandItem(), FakePlayerProvider.NULL_PROFILE)) {
    ...
    private float getBreakSpeed(EntityRobotBase robot, ItemStack usingItem, BlockState state, BlockPos pos) {
        ...
        if (f > 1.0F) {
            ...
            if (i > 0) {
                ...
                boolean canHarvest = ForgeHooks.isToolEffective(robot.level, pos, usingItem);
        ...
        f = ForgeEventFactory.getBreakSpeed(BlockUtil.getFakePlayerWithTool((ServerWorld) robot.level, robot.getMainHandItem(), FakePlayerProvider.NULL_PROFILE), state,
                f, blockToBreak);
        return f < 0 ? 0 : f;
    }
