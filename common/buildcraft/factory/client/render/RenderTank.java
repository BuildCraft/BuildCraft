...
public class RenderTank extends CullTESR<TileTank> {
    ...
    private static final Vec3d MAX_NORMAL = new Vec3d(14 / 16D - 0.01, 16 / 16D - 0.01, 14 / 16D - 0.01);
    private static final Vec3d MAX_CONNECTED = new Vec3d(14 / 16D - 0.01, 16 / 16D, 14 / 16D - 0.01);
    ...
    public void renderTileEntityFast(TileTank tile, double x, double y, double z, float partialTicks, int destroyStage, VertexBuffer buffer) {
        ...
        FluidRenderer.renderFluid(FluidSpriteType.STILL, tile.tank, sideRender[EnumFacing.DOWN.ordinal()] ? MIN_NORMAL : MIN_CONNECTED, sideRender[EnumFacing.UP.ordinal()] ? MAX_NORMAL : MAX_CONNECTED, buffer, sideRender);
    }
}
