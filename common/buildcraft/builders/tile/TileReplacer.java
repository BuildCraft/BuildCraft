...
public class TileReplacer extends TileBC_Neptune implements ITickable {
    ...
    public void update() {
        ...
        if (!invSnapshot.getStackInSlot(0).isEmpty() &&
            ...
            Snapshot.Header header = BCBuildersItems.snapshot.getHeader(invSnapshot.getStackInSlot(0));
            if (header != null) {
                ...
                if (snapshot instanceof Blueprint) {
                    ...
                    try {
                        ...
                        newBlueprint.header.id = UUID.randomUUID();
                        newBlueprint.header.owner = getOwner().getId();
                        newBlueprint.header.created = new Date();
//                        newBlueprint.header.name = name;
                        GlobalSavedDataSnapshots.get(world).snapshots.add(newBlueprint);
