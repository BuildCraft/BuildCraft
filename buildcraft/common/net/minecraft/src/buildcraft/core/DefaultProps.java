package net.minecraft.src.buildcraft.core;

public class DefaultProps {

	public static int WOODEN_GEAR_ID   = 3000;
	public static int STONE_GEAR_ID    = 3001;
	public static int IRON_GEAR_ID     = 3002;
	public static int GOLDEN_GEAR_ID   = 3003;
	public static int DIAMOND_GEAR_ID  = 3004;
	public static int TEMPLATE_ITEM_ID = 3005;
	
	public static int WOODEN_PIPE_ID      = 145;
	public static int STONE_PIPE_ID       = 146;
    public static int IRON_PIPE_ID        = 147;
    public static int GOLDEN_PIPE_ID      = 148;
    public static int DIAMOND_PIPE_ID     = 149;
    public static int MINING_WELL_ID      = 150;
    public static int DRILL_ID            = 151;
    public static int AUTO_WORKBENCH_ID   = 152;
    public static int QUARRY_ID           = 153;
    public static int MARKER_ID           = 154;
    public static int FILLER_ID           = 155;
    public static int OBSIDIAN_PIPE_ID    = 156;
    public static int BUILDER_ID          = 157;
    public static int TEMPLATE_ID         = 158;
    public static int COBBLESTONE_PIPE_ID = 159;
    public static int FRAME_ID            = 160;
	
	public static boolean CURRENT_CONTINUOUS   = false;
	public static boolean PIPES_ALWAYS_CONNECT = false;
	
}
