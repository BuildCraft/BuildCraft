...
public class BlockMiner {
    ...
    public void mineStack(ItemStack stack) {
        ...
        if (stack.stackSize > 0) {
            stack.stackSize -=
                Utils.addToRandomInjectableAround(owner.getWorldObj(), owner.xCoord, owner.yCoord, owner.zCoord, EnumFacing.UNKNOWN, stack);
        }
    ...
    public int acceptEnergy(int offeredAmount) {
        ...
        if (energyAccepted >= energyRequired) {
            ...
            BlockEvent.BreakEvent breakEvent =
                new BlockEvent.BreakEvent(x, y, z, world, block, meta, CoreProxy.proxy.getBuildCraftPlayer((WorldServer) world).get());
