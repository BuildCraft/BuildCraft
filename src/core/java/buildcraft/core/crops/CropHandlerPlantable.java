...
public class CropHandlerPlantable implements ICropHandler {
    ...
    public boolean canSustainPlant(World world, ItemStack seed, int x, int y, int z) {
        if (seed.getItem() instanceof IPlantable) {
            return world.getBlock(x, y, z).canSustainPlant(world, x, y, z, EnumFacing.UP, (IPlantable) seed.getItem())
                && world.isAirBlock(x, y + 1, z);
        } else {
            Block block = world.getBlock(x, y, z);
            IPlantable plantable = (IPlantable) ((ItemBlock) seed.getItem()).field_150939_a;
            return block.canSustainPlant(world, x, y, z, EnumFacing.UP, plantable) && block != ((ItemBlock) seed.getItem()).field_150939_a
                && world.isAirBlock(x, y + 1, z);
        }
    }
    ...
    public boolean isMature(IBlockAccess blockAccess, Block block, int meta, int x, int y, int z) {
        if (block == null) {
            return false;
        } else if (block instanceof BlockFlower || block instanceof BlockTallGrass || block instanceof BlockMelon || block instanceof BlockMushroom
